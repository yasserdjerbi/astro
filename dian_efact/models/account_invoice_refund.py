...
class AccountInvoiceRefund(models.TransientModel):
    ...
    date_invoice = fields.Date(
        string='Fecha de la nota', default=fields.Date.context_today, required=True)
    date = fields.Date(string='Fecha Contable')
    description = fields.Char(
        string='Motivo', required=True, default=_get_reason)
    refund_only = fields.Boolean(
        string='Technical field to hide filter_refund in case invoice is partially paid', compute='_get_refund_only')
    ...
    dian_note = fields.Selection(
        [('07', 'Crédito'), ('08', 'Débito')], string='Nota', default='07')
    CD_VALUES = [('1', 'Devolución de parte de los bienes; no aceptación de partes del servicio'), ('2', 'Anulación de factura electrónica'),
                 ('3', 'Rebaja total aplicada'), ('4', 'Descuento total aplicado'), ('5', 'Rescisión: nulidad por falta de requisitos'), ('6', 'Otros')]
    credit_discrepance = fields.Selection(
        CD_VALUES, string='Discrepancia', default='1')
    DB_VALUES = [('1', 'Intereses'), ('2', 'Gastos por cobrar'),
                 ('3', 'Cambio del valor')]
    debit_discrepance = fields.Selection(
        DB_VALUES, string='Discrepancia', default='1')
    ...
    def compute_refund(self, mode='refund'):
        ...
        for form in self:
            ...
            for inv in inv_obj.browse(context.get('active_ids')):
                ...
                if inv.reconciled and mode in ('cancel', 'modify'):
                    raise UserError(
                        _('Cannot create a credit note for the invoice which is already reconciled, invoice should be unreconciled first, then only you can add credit note for this invoice.'))
                ...
                if mode in ('cancel', 'modify'):
                    ...
                    if mode == 'modify':
                        ...
                        body = _('Correction of <a href=# data-oe-model=account.invoice data-oe-id=%d>%s</a><br>Reason: %s') % (
        ...
        if xml_id:
            ...
            if mode == 'modify':
                ...
                invoiceNote = self.env['account.invoice'].browse(
                    int(new_note_id))
                invoiceOrigin = self.env['account.invoice'].browse(
                    int(invoiceNote.refund_invoice_id))
                ...
                if(self.dian_note == "07"):
                    ...
                    invoiceNote.update({
                                        ...
                                        "type": "out_refund", 
                                        "is_exportation":invoiceOrigin.is_exportation,
                                        "calculation_rate":invoiceOrigin.calculation_rate,
                                        "exporting_currency":invoiceOrigin.exporting_currency,
                                        "dian_payment_means_code":invoiceOrigin.dian_payment_means_code,
                                        "dian_operation_type":invoiceOrigin.dian_operation_type,
                                        "dian_payment_means_id":invoiceOrigin.dian_payment_means_id,
                                       })

                if(self.dian_note == "08"):
